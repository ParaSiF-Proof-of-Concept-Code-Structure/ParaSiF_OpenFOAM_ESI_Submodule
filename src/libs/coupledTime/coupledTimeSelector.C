...
Foam::coupledTimeSelector::coupledTimeSelector(const std::string& str)
:
    scalarRanges(str)
{}
...
bool Foam::coupledTimeSelector::selected(const instant& value) const
{
    return scalarRanges::match(value.value());
}


Foam::List<bool> Foam::coupledTimeSelector::selected(const instantList& times) const
{
    ...
    for (const scalarRange& range : *this)
    {
        if (range.single())
        {
            ...
            const label nearestIndex =
                CoupledTimePaths::findClosestTimeIndex(times, target);
...
Foam::instantList Foam::coupledTimeSelector::select(const instantList& times) const
{
    return subset(selected(times), times);
}


void Foam::coupledTimeSelector::inplaceSelect(instantList& times) const
{
    inplaceSubset(selected(times), times);
}


void Foam::coupledTimeSelector::addOptions
(
    const bool constant,
    const bool withZero
)
...
Foam::instantList Foam::coupledTimeSelector::select
(
...
)
{
    if (times.size())
    {
        ...
        if (args.found("time"))
        {
            ...
            selectTimes = coupledTimeSelector(args["time"]).selected(times);
        }
...
Foam::instantList Foam::coupledTimeSelector::select0
(
    CoupledTime& runTime,
    const argList& args
)
...
Foam::instantList Foam::coupledTimeSelector::selectIfPresent
(
    CoupledTime& runTime,
    const argList& args
)
{
    ...
    // No coupledTimeSelector option specified. Do not change runTime.
